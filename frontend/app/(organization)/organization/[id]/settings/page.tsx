...
function OrganizationSettingsContent({ orgId }: { orgId: string }) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const org = await fetchOrganization(Number(orgId));
  ...
  async function onRecreateCode() {
    setSaving(true);
    try {
      const res = await recreateOrganizationInviteCode(Number(orgId));
      setInviteCode(res.inviteCode);
      toast.success("Новый код приглашения создан");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function reloadMembers() {
    const rows = await fetchOrganizationMembers(Number(orgId));
    setMembers(rows);
  }
  ...
  async function onRemoveMember(member: OrganizationMemberDto) {
    setSaving(true);
    try {
      await removeOrganizationMember(Number(orgId), member.userId);
      await reloadMembers();
  ...
  return (
    <>
      ...
      <Text className="mt-2 text-muted-foreground">
        Управление клубом. ID: <span className="font-mono">{orgId}</span>
      </Text>
      ...
      <Card className="mt-6 border-destructive/30 bg-destructive/5">
        ...
        <CardContent>
          <Text className="mb-3 text-sm text-muted-foreground">
            Удаление организации выполняется мягко: игры и история сохраняются,
            но карточка организации скрывается.
          </Text>
          <Button
            variant="destructive"
            disabled={saving}
            onClick={() => void onDeleteOrganization()}
          >
            Удалить организацию
          </Button>
        </CardContent>
    ...
    <Section>
      ...
      {id ? (
        <OrgAdminGate orgId={id}>
          <OrganizationSettingsContent orgId={id} />
        </OrgAdminGate>
      ) : (
